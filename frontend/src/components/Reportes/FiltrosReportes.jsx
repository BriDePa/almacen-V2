...
const FiltrosReportes = ({
  ...
  useEffect(() => {
    ...
    const marcasUnicas = [
      ...new Set(materiales.map((m) => m.marca).filter(Boolean)),
    ];
    const usosUnicos = [
      ...new Set(materiales.map((m) => m.uso).filter(Boolean)),
    ];
    ...
    <div className="card">
      <form onSubmit={handleSubmit}>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div>
            ...
            >
              <option value="">Todos</option>
              {[
                ...new Set(materiales.map((m) => m.nombre).filter(Boolean)),
              ].map((nombre) => (
        ...
        <div className="flex justify-between items-center mt-6">
          ...
          {/* Botón de imprimir (si está habilitado por el padre) */}
